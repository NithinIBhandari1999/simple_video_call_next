...
const VideoCall = (props) => {
	...
	const listenConnection = () => {
		try {
			...
			socketObj.on(constantSocketActions.ROOM_JOINED, (args) => {
				try {
					console.log('Event Type: ', constantSocketActions.ROOM_JOINED);
		...
		const sendOfferToOtherUsers = (offer) => {
			try {
				...
				if (typeof offer === 'string' && offer !== '') {
					...
					console.error('Not sending request as offer is empty. Offer=', offer);
					return;
				}
			...
			peerConnection.onicecandidate = async (event) => {
				try {
					...
					if (event?.candidate) {
						...
						if (localDescription) {
							sendOfferToOtherUsers(JSON.stringify(localDescription));
						}
					}
				} catch (error) {
					console.log(error);
	...
	const createAnswer = async (offer) => {
		try {
			...
			peerConnection.onicecandidate = async (event) => {
				...
				if (event.candidate) {
					let jsonLocalDescription = JSON.stringify(peerConnection?.localDescription);
